...
void MyRobot::follow_wall()
{
    ...
    {
            // Wall following
        if (_critical_mode == false)
        {
            if ((_dist_val[1] > FOLLOWING_DISTANCE) || (_dist_val[14] > FOLLOWING_DISTANCE))
            {
                if(_dist_val[1] > _dist_val[14])
                {
                    if(_dist_val[7] < CRITICAL_DISTANCE || _dist_val[7] < _dist_val[13])
                    {
                        _mode = WALL_FOLLOWER_LEFT;
                    ...
                    {
                        _mode = TURN_RIGHT;
                        cout << "TURN_RIGHT-B" << endl;
                    }
                }

                else
                {
                    if(_dist_val[8] < CRITICAL_DISTANCE || _dist_val[8] < _dist_val[2])
                    {
                        _mode = WALL_FOLLOWER_RIGHT;
                 ...
                 if ((_dist_val[13] > FOLLOWING_DISTANCE || _dist_val[2] < FOLLOWING_DISTANCE + 50)) {
                        _mode = TURN_LEFT;
                        cout << "TURN_LEFT" << endl;
                    }
                    else {
                        if (_dist_val[13] < FOLLOWING_DISTANCE + 50 || _dist_val[2] > FOLLOWING_DISTANCE ) {
                            _mode = TURN_RIGHT;
            ...
            if ((_dist_val[13] > FOLLOWING_DISTANCE && _dist_val[2] > FOLLOWING_DISTANCE) || (_dist_val[3] > FOLLOWING_DISTANCE && _dist_val[14] > FOLLOWING_DISTANCE) || (_dist_val[1] > FOLLOWING_DISTANCE && _dist_val[12] > FOLLOWING_DISTANCE))
            {
                _mode = TURN_OVER_EDGE;
                cout << "TURN_OVER_EDGE" << endl;
                _critical_mode = true;
            }
        }
        else
        {
            if (_dist_val[0] < FOLLOWING_DISTANCE && _dist_val[13] < FOLLOWING_DISTANCE)
